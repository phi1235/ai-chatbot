...
class GateRunRequest(BaseModel):
    trigger_source: str = "manual"   # manual | nightly | ci
...
async def list_gate_configs(
    ...
    """List eval gate configs. Optionally filter by kind or enabled."""
...
async def patch_gate_config(config_id: int, req: GateConfigPatchRequest):
    """Partially update an eval gate config."""
    from orchestrator import eval_gate_store
...
async def run_gate_config(config_id: int, req: GateRunRequest):
    """Manually trigger a gate run for a specific config.

    Runs a full batch, resolves a baseline, compares, applies thresholds,
    persists a gate run record, and returns the gate run with decision.
    """
    from orchestrator import eval_gate_store
...
async def list_gate_runs(
    ...
    """List gate runs. Newest first. Optionally filter by config, kind, or decision."""
    from orchestrator import eval_gate_store
...
async def run_nightly_gates():
    ...
    Intended to be called nightly by a cron/scheduler.
    Returns a compact summary of each gate run and its decision.
    """
...
async def run_ci_gate(req: RunCIRequest):
    """Run a named CI gate config.

    Intended to be called from CI pipelines (e.g. GitHub Actions).
    Returns the gate run dict; HTTP 200 regardless of decision so CI can
    inspect the response. Use the 'decision' field to decide whether to fail
    the pipeline step.

    For shell scripts that need a non-zero exit code on failure, use the CLI:
